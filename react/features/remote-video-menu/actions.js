...
/**
 * Tags the remote participant with the given name.
 *
 * @param {string} participantId - ID of the participant to mute.
 * @returns {Function}
 */
export function tagParticipant(participantId: string, tagName: string) {
    return (dispatch: Dispatch<any>) => {
        //sendAnalytics(createRemoteMuteConfirmedEvent(participantId));
        dispatch(tagRemoteParticipant(participantId, tagName));
    };
}
