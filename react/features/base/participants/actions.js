...
/**
 * Create an action for taging a participant in the conference.
 *
 * @param {string} id - Participant's ID.
 * @param {TAG_NAME} role - Participant's new role.
 * @returns {{
 ...
 * }}
 */
export function tagRemoteParticipant(id, tag) {
    //let partcipant = getParticipantDisplayName(id);
    //console.log("tagRemoteParticipant")
    //console.log(JSON.stringify(partcipant))
...
export function participantUpdated(participant = {}) {
    ...
    if (participant.tag) {
        participantToUpdate.tag = participant.tag;
    }
